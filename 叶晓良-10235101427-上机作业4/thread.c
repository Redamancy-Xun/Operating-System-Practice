...
thread_unblock (struct thread *t) 
{
  ...
  list_insert_ordered (&ready_list, &t->elem, less_by_priority, NULL);
  t->status = THREAD_READY;
  intr_set_level (old_level);
}

bool
less_by_priority (const struct list_elem *a, const struct list_elem *b, void *aux)
{
...
init_thread (struct thread *t, const char *name, int priority)
{
  ...
  old_level = intr_disable ();
  list_insert_ordered (&all_list, &t->allelem, less_by_priority, NULL);
  intr_set_level (old_level);
}
